...
from .utils.token import MytokenSerializer
from .utils.vecode import send_email



# JWT
def get_token_for_user(user, *args):
...
class DeveloperRegistrationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"email": email, "otp": otp}, status=status.HTTP_201_CREATED
            )
        return Response(
...
class VendorRegistrationView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            encode_header = base64.urlsafe_b64encode(
                json.dumps({"alg": "HS256", "typ": "JWT"}).encode()
            ).decode()
            encode_payload = base64.urlsafe_b64encode(
            ...
            send_email(data)
            return Response(
                {"data": data, "token": token}, status=status.HTTP_201_CREATED
            )
...
class VerifyEmail(generics.GenericAPIView):
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"msg": "You Already verified Your Email"},
...
class LoginOtpverification(APIView):
    def post(self, request):
        ...
        if int(user.otp) == int(entered_otp):
            token = get_token_for_user(user)
            return Response(
                {"msg": "User loggined successfully", "token": token},
                status=status.HTTP_200_OK,
            )
